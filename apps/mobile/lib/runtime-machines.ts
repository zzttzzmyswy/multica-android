/**
 * Machine grouping for the workspace runtimes list (iteration-83, A2.4) —
 * ports web `packages/views/runtimes/components/runtime-machines.ts` semantics
 * (Grouping by daemon/device into RuntimeMachine rows with local/remote/cloud
 * sections, health derivation, online/issue/running/queued counts) plus
 * `machineUpdateRuntime` (machine-cli-section.tsx), the local
 * `formatDeviceInfo` (web `runtimes/utils.ts`) and `buildWorkloadIndex`
 * (web runtime-list.tsx) so the phone list can group machines exactly the way
 * web does.
 *
 * Pure TS + `@multica/core` primitives only — the vitest lane exercises every
 * branch without RN.
 */
...
export interface RuntimeMachineOptions {
  now: number;
  ...
  /**
   * Desktop synthesizes a placeholder local machine when the local daemon
   * row is missing (Start button etc.). Mobile omits it.
   */
...
function currentMachineMetadata(
  ...
  for (const runtime of [...candidates].sort(compareRuntimeReports)) {
...
/** Read a single string metadata field off one runtime row (null-safe). */
...
// ---------------------------------------------------------------------------
// Formatting + command-channel selection (web runtimes/utils.ts +
// machine-cli-section.tsx) — kept here so the machine cards and the detail
// update section share one implementation.
// ---------------------------------------------------------------------------

// Turns the back-end's `device_info` string ("MacBook-Pro · darwin-amd64",
// "some-host · linux-amd64") into something humans recognise by rewriting the
// GOOS/GOARCH suffix while preserving the hostname.
...
export function buildWorkloadIndex(
  agents: Agent[],
  tasks: AgentTask[],
): Map<string, RuntimeWorkloadSummary> {
  const result = new Map<string, RuntimeWorkloadSummary>();
