...
describe("workspace billing i18n", () => {
  ...
  const EN_ZH: Record<string, [string, string]> = {
    ...
    "billing.currentPlanDescription": ["The entitlement currently enforced for this workspace.", "这个工作区当前生效的权益。"],
    ...
    "billing.limitsIssues": ["Recent tasks", "最近活跃的 task"],
    "billing.limitsIssuesDescription": ["Free keeps the most recently active tasks available. Pro has no window limit.", "Free 可访问最近活跃的 task；Pro 没有窗口限制。"],
    ...
    "billing.seatsHumanMembersDescription": ["Refresh after membership changes to ask cloud billing to reconcile Stripe quantity.", "成员变化后刷新，通知 Cloud Billing 对账 Stripe 数量。"],
