/**
 * Pull-request status derivation — ported from web's
 * `packages/core/github/pull-request-status.ts` so the PR listing on the
 * issue detail page renders the same CI / mergeability verdicts as web
 * (Behavioral parity rule in apps/mobile/CLAUDE.md). Keep this file in sync
 * with the web original; the test suite mirrors the web suite.
 *
 * The PR row surfaces TWO independent facts, each tri-state and each sourced
 * from the GitHub API snapshot:
 *
 *   1. CI status      — derived from `checks_rollup` (primary) + counts.
 *   2. Mergeability   — derived from `mergeable` + `merge_state_status`.
 *
 * The two are intentionally decoupled: a PR can have failing checks AND a
 * merge conflict, and both must show. Neither element is derived from the
 * other, and neither is shown for terminal PRs (merged / closed) — the row's
 * leading state icon already conveys terminal state; the caller applies that
 * gate.
 *
