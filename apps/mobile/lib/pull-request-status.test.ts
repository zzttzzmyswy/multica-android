/**
 * Unit tests for `pull-request-status`, the PR listing derived-status logic
 * ported from web's `packages/core/github/pull-request-status.ts`.
 *
 * Every case below mirrors the web suite verbatim so mobile and web stay
 * behaviorally aligned (Behavioral parity rule in apps/mobile/CLAUDE.md):
 * CI status and mergeability are independent tri-state elements, snapshot
 * fields are defensive (`?? 0`, `?? []`, explicit `===` checks), and a
 * missing field never fabricates a positive verdict.
 */
