...
describe("dashboard usage api methods (iteration-87 project filter)", () => {
  it("getDashboardUsageDaily GETs /api/dashboard/usage/daily?days=7&tz=UTC", async () => {
    const spy = fetchSpy().mockResolvedValue([
      { date: "2026-08-15", input_tokens: 1000, output_tokens: 200, task_count: 3 },
    ]);
    const res = await api.getDashboardUsageDaily(7, null, "UTC");
    expect(spy).toHaveBeenCalledWith(
      "/api/dashboard/usage/daily?days=7&tz=UTC",
  ...
  it("getDashboardUsageDaily honours the abort signal", async () => {
    const spy = fetchSpy().mockResolvedValue([]);
    await api.getDashboardUsageDaily(30, null, "UTC", { signal: undefined });
    expect(spy).toHaveBeenCalledWith(
      "/api/dashboard/usage/daily?days=30&tz=UTC",
      expect.objectContaining({ signal: undefined }),
    );
  });
  ...
  it("getDashboardUsageDaily appends project_id when a project is selected", async () => {
    const spy = fetchSpy().mockResolvedValue([]);
    await api.getDashboardUsageDaily(7, "project-1", "UTC");
    expect(spy).toHaveBeenCalledWith(
      "/api/dashboard/usage/daily?days=7&project_id=project-1&tz=UTC",
      expect.objectContaining({ signal: undefined }),
    );
  });
  ...
  it("getDashboardUsageByAgent GETs /api/dashboard/usage/by-agent?days=30&tz=UTC", async () => {
    const spy = fetchSpy().mockResolvedValue([
      { agent_id: "agent-1", input_tokens: 500, task_count: 2 },
    ]);
    const res = await api.getDashboardUsageByAgent(30, null, "UTC");
    expect(spy).toHaveBeenCalledWith(
      "/api/dashboard/usage/by-agent?days=30&tz=UTC",
      expect.objectContaining({ signal: undefined }),
    );
    expect(res).toHaveLength(1);
  ...
  it("getDashboardUsageByAgent appends project_id when a project is selected", async () => {
    const spy = fetchSpy().mockResolvedValue([]);
    await api.getDashboardUsageByAgent(7, "project-2", "UTC");
    expect(spy).toHaveBeenCalledWith(
      "/api/dashboard/usage/by-agent?days=7&project_id=project-2&tz=UTC",
      expect.objectContaining({ signal: undefined }),
    );
  });
  ...
  it("getDashboardUsageByAgent degrades a drift response to []", async () => {
    ...
    const res = await api.getDashboardUsageByAgent(7, null, "UTC");
    expect(res).toEqual([]);
  });
});