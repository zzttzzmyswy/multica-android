...
集成最完整的一款。会话续接好用，MCP 好用，是真正会读 agent 配置里 `mcp_config` 字段的两款工具之一（另一款是 OpenCode，见[矩阵](/zh/providers)）。
...
开源 CLI agent。会从自己的配置文件里动态发现可用模型 —— 适合想自己掌控模型清单的用户。会消费 agent 配置里的 `mcp_config` 字段（12 款里和 Claude Code 一起的两款之一）。
