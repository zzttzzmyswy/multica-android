...
`GITHUB_APP_ID` 和 `GITHUB_APP_PRIVATE_KEY` 同时用于 **PR 卡片的 CI 状态与可合并性**和 **Settings → Repositories → 从 GitHub 选择**。它们让 Multica 以 App 身份鉴权（App JWT → installation token）、拉取 PR 卡片依赖的 GitHub API 快照，并签发仅有 metadata read 权限的短效 token 来列出 installation 的仓库。私钥从 App 设置页 **Private keys → Generate a private key** 生成，把整段 PEM（含 `-----BEGIN/END RSA PRIVATE KEY-----` 两行）粘到 env 里，保留换行。不配这两个时集成会**干净降级**：PR 照常镜像、issue 照常自动关联并在 merge 时转 Done、关联账号名仍由 `installation.created` webhook 解析——但 PR 卡片不显示 CI/merge 状态，仓库浏览也会被禁用。
