...
特定の Multica リリースに移行するには、values ファイルで `images.backend.tag` / `images.frontend.tag` を設定して `helm upgrade` を実行してください — タグが変わることで再取得とロールアウトが強制されるため、アップグレードもこの経路を使います。`kubectl -n multica rollout restart deploy/multica-backend deploy/multica-frontend` が浮動タグを取り直すのは、`images.backend.pullPolicy` / `images.frontend.pullPolicy` を `Always` に設定している場合だけです。チャートのデフォルトは `IfNotPresent` で、ノードがすでにキャッシュしているイメージを再利用します。`helm -n multica uninstall multica` はワークロードを削除しますが、PVC と Secret は保持します。`kubectl delete namespace multica` はすべてを消去します。
