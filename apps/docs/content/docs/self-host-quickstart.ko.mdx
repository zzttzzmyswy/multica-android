...
특정 Multica 릴리스로 옮기려면 values 파일에서 `images.backend.tag` / `images.frontend.tag`를 설정하고 `helm upgrade`를 실행하세요 — 태그가 바뀌어야 재다운로드와 롤아웃이 강제되므로 업그레이드도 이 경로를 씁니다. `kubectl -n multica rollout restart deploy/multica-backend deploy/multica-frontend`가 부동 태그를 다시 받아오는 것은 `images.backend.pullPolicy` / `images.frontend.pullPolicy`를 `Always`로 설정한 경우뿐입니다. 차트 기본값은 `IfNotPresent`라서 노드에 이미 캐시된 이미지를 재사용합니다. `helm -n multica uninstall multica`는 워크로드를 제거하지만 PVC와 Secret은 유지합니다. `kubectl delete namespace multica`는 모든 것을 삭제합니다.
