...
その URL に任意の JSON を POST してください。Multica はまず配信を永続化し、冪等な実行を同期的に受け付け、`status = accepted | skipped` と `run_id` を含む `200 OK` を返します。その後データベース-backed worker が accepted の実行を再開してエージェントをディスパッチします。受付とディスパッチの間にサーバーが再起動しても、キュー済みの配信は失われません。
...
GitHub の配信は `X-GitHub-Delivery` で重複排除されます。汎用の送信元は
`Idempotency-Key` を指定できます。同じキーで再試行すると、元のキュー済み配信と
run が再利用されます。どちらのヘッダーもない場合は安定したイベント ID がないため、
重複排除はベストエフォートになります。
...
有効でアクティブな配信は、配信と冪等な実行の両方が永続化された後に `200 OK` で確認されます。

- `{"status":"accepted","delivery_id":"…","run_id":"…","autopilot_id":"…","trigger_id":"…"}` — 受付が成功し、worker が復旧可能なディスパッチを所有します。
- `{"status":"skipped","delivery_id":"…","run_id":"…","reason":"…"}` — たとえば担当エージェントのランタイムがオフラインの場合に、永続化された skipped の実行が作成されました。

no-op と重複も `200 OK` を返します。

- `{"status":"duplicate","delivery_id":"…","run_id":"…"}` — 同じ冪等キーの配信は既に受け付け済みです。元のリクエストが実行の受付前に終了した場合のみ `run_id` はなく、worker が復旧します。
...
- `401` — 設定済みの webhook 署名がないか無効です。
- `404` — 不明なトークン（`{"error":"webhook not found"}`）。
- `413` — payload が 256 KiB を超えました。
- `429` — IP の絶対上限、または不明なトークン / 無効な署名の試行上限を超えました。応答には `Retry-After` が含まれます。有効な配信は worker のトリガー別予算によって拒否されず、予算が戻るまでキューに残ります。
