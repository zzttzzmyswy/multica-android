...
その URL に任意の JSON を POST してください。Multica はまず配信を永続化して `202 Accepted` を返し、その後データベース-backed worker が `source = webhook` の実行を記録してエージェントをディスパッチします。受付とディスパッチの間にサーバーが再起動しても、キュー済みの配信は失われません。
...
有効でアクティブな配信は、永続化された後に `202 Accepted` で確認されます。`{"status":"queued","delivery_id":"…"}` は worker がディスパッチを所有したことを示します。no-op と重複は `200 OK` を返します。

- `{"status":"duplicate","delivery_id":"…","run_id":"…"}` — 同じ冪等キーの配信は既に受け付け済みです。元の配信がまだキュー中なら `run_id` はありません。
