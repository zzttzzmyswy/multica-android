...
- 省略号：用 `...`（三点）而非 `…`（单字符），与英文 source 保持一致
  - **本条自相矛盾，两个 bundle 已经因此各走一边。** 英文 source 两种都用（移动端 bundle `…` 96 / `...` 16，views `…` 129 / `...` 106），所以「用三点」和「与英文 source 保持一致」不可能同时成立。目前移动端 zh 跟 source（仅 6 条不一致），views zh 跟本条前半句（52 条用 `...` 而英文用 `…`）。两侧都没有守卫钉住，也没有任何一轮被授权二选一。翻译前先把这一条定下来。
