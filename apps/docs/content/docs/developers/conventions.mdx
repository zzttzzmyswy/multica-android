...
- **Figure and counter.** ko writes `45초`, `5분`, `30일`, `12건` and never
  `45 초` — 37 tight occurrences against 0 spaced, so it is pinned. ja is *not*
  settled; see the table below.
...
records the competing forms, the two same-kind keys that take different ones, and
fails if a later round makes the split disappear without settling the term.

| Term | Where | Why the bundle cannot settle it |
| --- | --- | --- |
| `Server` | `agents.tab_body.mcp_config.*` | 23 Latin vs 15 サーバー / 서버 inside that one surface, alternating key by key, while `settings.mcp.*` is native in all 15 of its strings |
| `review` | ko | 검토 9 vs 리뷰 8, and the status control (`issues.status.in_review`) disagrees with the prose that enumerates the same statuses |
| `label` | ko | 라벨 26 vs 레이블 25, no partition — the same modal takes both (`modals.create_issue.set_labels` is 레이블, its sibling toast is 라벨) |
| `Roles in ja/ko prose` | ja / ko | ja keeps lowercase Latin roles in permission prose but renders the role labels natively; ko splits inside one surface (`settings.mcp.admin_only_note` is Latin, `settings.workspace.manage_hint` is native). The section above does not say which applies |
| `Register (polite level)` | ko | 856 values carry 습니다 and 398 an 해요체 ending, with no clean partition — `common.lark_bind.error_expired` and `common.slack_bind.error_expired` are the same string in two registers. See the note below |
| `Private` | ja | 7 プライベート vs 9 非公開, split by neither surface nor label-vs-prose: `settings.plugins.private` and `settings.repositories.github_private` are both a bare `Private` label under settings and take different words |
| `Figure and counter spacing` | ja | 42 spaced occurrences vs 14 tight on a literal figure (155 vs 40 on an interpolation), and the same English source renders both ways: `autopilots.relative_date.one_day_ago` is `1 日前`, `projects.relative_date.one_day_ago` is `1日前` |
