...
| `GITHUB_APP_ID` | 비어 있음 | App 설정 페이지에 표시되는 숫자 App ID. `GITHUB_APP_PRIVATE_KEY`와 함께 PR 카드의 CI 상태와 머지 가능 여부, Settings → Repositories → GitHub에서 선택 기능, 설치 시 연결된 계정 이름 가져오기에 사용됩니다 |
| `GITHUB_APP_PRIVATE_KEY` | 비어 있음 | App RSA 비공개 키의 전체 PEM 블록(`-----BEGIN/END-----` 줄 포함, 줄바꿈 유지). `GITHUB_APP_ID`와 함께 App 인증 PR 스냅샷과 저장소 탐색에 사용됩니다 |
...
- 서버가 저장소 목록에 필요한 installation token을 발급할 수 없으므로 Settings → Repositories → GitHub에서 선택 기능이 비활성화됩니다.
