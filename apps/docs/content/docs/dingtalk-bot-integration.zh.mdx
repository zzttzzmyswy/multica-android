...
| **发图片** | 单聊里的图片、或群里随 @ 一起发的图片，都会进入对话让智能体看到——支持 PNG、JPEG、GIF、WebP、BMP，每条消息最多 4 张、每张不超过 10 MB。每张图片都会复制进 Multica 存储，所以钉钉的临时链接过期后，它在对话里依然可见。文件和语音不支持。 |
| **`/issue` 命令** | 以 `/issue <标题>` 开头，会按输入直接、同步地以你的身份创建 Multica issue，并在原对话中返回 issue 编号和标题；后续行会作为描述。同一条钉钉消息中的图片只附在聊天消息上，不会复制到直接创建的 issue。 |
