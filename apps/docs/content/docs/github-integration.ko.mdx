...
`GITHUB_APP_ID`와 `GITHUB_APP_PRIVATE_KEY`는 **PR 카드의 CI 상태와 머지 가능 여부** 및 **Settings → Repositories → GitHub에서 선택** 기능 모두에 필요합니다. 이 둘로 Multica가 App으로 인증하고(App JWT → installation token), PR 카드의 GitHub API 스냅샷을 가져오며, metadata read로 제한된 단명 token으로 installation의 저장소를 나열합니다. 비공개 키는 App 설정 페이지의 **Private keys → Generate a private key**에서 생성한 뒤, PEM 블록 전체(`-----BEGIN/END RSA PRIVATE KEY-----` 줄 포함)를 줄바꿈을 유지한 채 env 값에 붙여넣으세요. 이 둘이 없으면 연동은 **깔끔하게 축소**됩니다: PR은 계속 미러링되고, 이슈는 계속 자동 연결되며 머지 시 Done으로 이동하고, 연결된 계정 이름도 `installation.created` 웹훅에서 확인됩니다 — 다만 PR 카드에는 CI나 머지 상태가 표시되지 않고 저장소 탐색도 비활성화됩니다.
