...
그 URL로 아무 JSON이나 POST하세요. Multica는 먼저 전달을 영구 저장하고 `202 Accepted`를 반환한 뒤, 데이터베이스-backed worker가 `source = webhook` 실행을 기록하고 에이전트를 디스패치합니다. 수신과 디스패치 사이에 서버가 재시작되어도 큐에 저장된 전달은 유실되지 않습니다.
...
유효하고 활성 상태인 전달은 영구 저장된 뒤 `202 Accepted`로 확인됩니다. `{"status":"queued","delivery_id":"…"}`는 worker가 디스패치를 맡았음을 뜻합니다. no-op과 중복은 `200 OK`를 반환합니다.

- `{"status":"duplicate","delivery_id":"…","run_id":"…"}` — 같은 멱등 키의 전달이 이미 수신되었습니다. 원본 전달이 아직 큐에 있으면 `run_id`는 생략됩니다.
