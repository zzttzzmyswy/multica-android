...
向这个 URL POST 任意 JSON。Multica 会先持久化投递并返回
`202 Accepted`，再由数据库 worker 记录一条 `source = webhook` 的 run，
把请求体保存为 run 的 `trigger_payload`，然后按和 schedule 触发器完全
一致的方式派发给智能体。接收与派发之间即使服务重启，已排队的投递也不会丢失。
