...
向这个 URL POST 任意 JSON。Multica 会先持久化投递，并同步准入一条
幂等 run，返回带有 `status = accepted | skipped` 和 `run_id` 的 `200 OK`。
数据库 worker 随后恢复 accepted run，把请求体保存为 run 的
`trigger_payload`，再按和 schedule 触发器完全一致的方式派发给智能体。
接收与派发之间即使服务重启，已排队的投递也不会丢失。
...
GitHub 投递用 `X-GitHub-Delivery` 去重；通用发送方可以提供
`Idempotency-Key`。同一个 key 的重试会复用原来的排队投递和 run。不带
这两个 header 时，因为没有稳定的事件标识，投递只能提供尽力而为的语义。
...
有效且活跃的投递只有在投递和幂等 run 都持久化后才会确认：

- `200 {"status":"accepted","delivery_id":"…","run_id":"…","autopilot_id":"…","trigger_id":"…"}`
  —— 准入成功，后续派发由 worker 接管并可恢复。
- `200 {"status":"skipped","delivery_id":"…","run_id":"…","reason":"…"}`
  —— 准入生成了持久化的 skipped run，例如负责人运行时离线。

正常 no-op 和去重路径返回 `200 OK`，避免外部 webhook 无限重试：

- `{"status":"duplicate","delivery_id":"…","run_id":"…"}` —— 发送方的
  幂等 key 已经接收。只有原请求在 run 准入前中断时才不带 `run_id`；worker
  会恢复它。
...
- `401` —— 已配置的 webhook 签名缺失或无效。
- `404` —— 未知 token（`{"error":"webhook not found"}`）。
- `413` —— 请求体超过 256 KiB。
- `429` —— 命中高位 IP 绝对上限，或者该 IP 累积了过多未知 token / 无效
  签名请求；响应会带 `Retry-After`。worker 的单触发器预算不会拒绝已经接收的
  有效投递，只会让它继续排队，等预算恢复后再派发。
