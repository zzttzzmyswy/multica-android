...
| `GITHUB_APP_ID` | 空 | App 設定ページに表示される数値の App ID。`GITHUB_APP_PRIVATE_KEY` と併せて、PR カードの CI ステータスとマージ可否、Settings → Repositories → GitHub から選択する機能、インストール時の連携先アカウント名取得に使われます |
| `GITHUB_APP_PRIVATE_KEY` | 空 | App の RSA 秘密鍵の完全な PEM ブロック（`-----BEGIN/END-----` 行を含み、改行を保持）。`GITHUB_APP_ID` と併せて、App 認証による PR スナップショットとリポジトリ参照に使われます |
...
- サーバーがリポジトリ一覧に必要な installation token を発行できないため、Settings → Repositories → GitHub から選択する機能は無効になります。
