...
| `GITHUB_APP_ID` | empty | Numeric App ID from the App's settings page. Required (with `GITHUB_APP_PRIVATE_KEY`) for PR-card CI status and mergeability, Settings → Repositories → Choose from GitHub, and fetching the connected account name on install |
| `GITHUB_APP_PRIVATE_KEY` | empty | Full App RSA private-key PEM block (including `-----BEGIN/END-----` lines, newlines preserved). Required (with `GITHUB_APP_ID`) for App-authenticated PR snapshots and repository browsing |
...
- Settings → Repositories → Choose from GitHub is disabled because the server cannot mint the installation token needed to list repositories.
