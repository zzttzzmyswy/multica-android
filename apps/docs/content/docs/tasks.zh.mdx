...
上面这层是**服务端的粗粒度兜底**——按任务启动时间算，不看任务是否还在活动。真正区分「卡死」和「正常的长任务」的是**本地守护进程**：它不再用固定墙钟时长砍任务（`MULTICA_AGENT_TIMEOUT` 默认 `0` = 不设上限），而是看活动——只要 agent 还在持续产出事件（消息、工具调用），守护进程就不会因为跑得久判它超时（服务端那条 2.5h 仍是外层上限）。只有真正静默卡死时才会被**空闲看门狗**（`MULTICA_AGENT_IDLE_WATCHDOG`，默认 30 分钟）终止；OpenCode 还会用更短的 `MULTICA_OPENCODE_IDLE_WATCHDOG`（默认 10 分钟）识别首 token 未到或流式回复中断。如果是某个工具调用发出后长时间无任何输出（疑似卡死的子进程），则由更大的**工具看门狗**预算（`MULTICA_AGENT_TOOL_WATCHDOG`，默认 2 小时）兜底。这类被看门狗终止的任务失败原因是 `idle_watchdog`，和墙钟 `timeout` 区分开。各参数见 [环境变量 → 守护进程的调节参数](/environment-variables#守护进程的调节参数)。
...
- **复用工作目录；是否续接会话取决于你从哪里触发：**
  - **执行日志里某一行的 retry**（带 task ID）：复用那一次执行的工作目录，磁盘上已经写好的文件会被保留；同时也会续接那次执行的会话——**除非**这次失败污染了对话（context 溢出、迭代上限、invalid request、Codex 语义无活动、agent fallback），这种情况下会在复用的工作目录上开一个全新会话。如果原目录已经不在了（被 GC、在另一台 runtime 上、或失败太早还没记录），则回退到全新目录。这样一来，网络抖动、provider 5xx / 限流、runtime 短暂离线、或你已经修好的 auth/额度/配置错误等**临时故障**就能从中断处继续，而不用从头再来。
  - **不带 task ID 的 CLI / API rerun**：没有具体某一次执行可续，因此**开一个全新会话 + 全新工作目录**。
...
| 工作目录 | 复用（接着上次的文件继续） | 单行 retry：原目录还在就复用；CLI / 不带 task_id：全新 |
| 会话继承 | 是（接着上次会话） | 单行 retry：除非失败污染了对话，否则续接；CLI / 不带 task_id：全新 |
...
Multica 在任务过程中**两次**保存会话 ID——任务一开始（AI 工具返回第一条系统消息时）pin 一次，任务结束（完成或失败）时再 pin 一次。前者让守护进程中途崩溃时也能恢复，后者会被下一次**自动重试**以及**执行日志里的单行 retry**复用——把这个 ID 传回去，智能体就能接着上次的对话和文件状态继续；只有当这次失败污染了对话时，才只复用工作目录、另开全新会话。**不带 task ID 的 CLI / API rerun** 则从全新会话 + 全新工作目录开始——见 [手动重跑和自动重试的区别](#手动重跑和自动重试的区别)。
...
- [Providers Matrix](/providers) —— 16 款 AI 编程工具的能力差异对照（包括会话恢复的精确状态）
