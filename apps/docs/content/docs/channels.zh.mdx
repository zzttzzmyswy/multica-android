---
title: 聊天集成
description: 把 Multica 智能体接入飞书、Lark 或 Slack，在团队已有的聊天工具中使用。
---
...
目前支持飞书/Lark 和 Slack。两种集成使用相同的会话、身份和执行机制，但安装方式不同。

## 选择平台

| | 飞书 / Lark | Slack |
|---|---|---|
| 安装方式 | 在 Multica 中生成二维码，用飞书扫码授权 | 在 Slack 创建 app，再把两个 token 填入 Multica |
| 私聊智能体 | 支持 | 支持 |
| 群聊或频道 | @ Bot 后触发 | @ Bot 后触发 |
| 创建 issue | `/issue` 消息命令，按输入直接创建 | `/issue` slash 命令，智能体整理描述后创建 |
| 连接方式 | 平台长连接 | Socket Mode |
