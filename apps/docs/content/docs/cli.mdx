...
| | `mcp get/set/add/remove` | Manage the MCP servers shared by every agent in the workspace | Writes are owner/admin. `get` lists names and transports only — the stored configuration is write-only and never returned. `set` replaces the whole document (`--mcp-config-file` / `--mcp-config-stdin` keep tokens out of shell history; `--mcp-config null` clears it); `add <name>` / `remove <name>` change one server without holding the document |
