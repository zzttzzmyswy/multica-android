...
Deleting a runtime that still has active agents bound to it is refused by default. `multica runtime delete <id> --cascade` also archives those agents and cancels their active tasks.
...
| `runtime` | `list/usage/activity/update/rename/delete` | View and manage runtimes | `delete`: `--cascade` (also archives bound agents and cancels their tasks) |
