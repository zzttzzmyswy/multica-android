...
func init() {
	...
	agentCreateCmd.Flags().String("mcp-config", "", "MCP server configuration as a JSON object, e.g. '{\"mcpServers\":{\"shortcut\":{...}}}'. Authoritative: the agent sees exactly these servers and none of the runtime host's own, and '{\"mcpServers\":{}}' means no MCP at all. Omit the flag to inherit the host's servers instead, or set runtime_config.mcp.inherit_runtime=true to have both. Treated as secret material (MCP entries often carry API tokens) — never logged by the CLI, but values passed on the command line are visible to shell history and 'ps'; prefer --mcp-config-stdin or --mcp-config-file for real secrets.")
	...
	agentUpdateCmd.Flags().String("mcp-config", "", "New MCP server configuration as a JSON object, e.g. '{\"mcpServers\":{...}}'. Authoritative: the agent sees exactly these servers and none of the runtime host's own, and '{\"mcpServers\":{}}' means no MCP at all. Pass 'null' to clear and inherit the host's servers instead, or set runtime_config.mcp.inherit_runtime=true to have both. Treated as secret material — never logged by the CLI, but values passed on the command line are visible to shell history and 'ps'; prefer --mcp-config-stdin or --mcp-config-file for real secrets.")
